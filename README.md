...
By default, [MetaPerson Creator](https://metaperson.avatarsdk.com/) exports models in **FBX** format. However, you can also obtain models in GLB format using one of the following methods:

### Option 1: Export GLB Directly from MetaPerson Creator

You can export a model in **GLB** format directly from the MetaPerson Creator interface.
Simply open your model in the Creator, choose **GLB** as the export format, and download the resulting file.

### Option 2: Use Business Integration

If you’re using **MetaPerson Creator** as part of a business integration, you can also export models in **GLB** format programmatically:
...
## How To Reduce Data Size And Memory Usage
All MetaPerson avatars share the same topology, which makes it possible to load only the avatar’s head while reusing the existing body mesh.

1. Seet the [Head Replacement Sample](./Documentation~/MetaPersonHeadReplacementSample.md) for a demonstration of how to load **head-only** GLB models.

2. Check the [PointCloudLoading](./Documentation~/MetaPersonPointCloudLoadingSample.md) sample to learn how to load models from point clouds generated by the **Standalone Application**.
